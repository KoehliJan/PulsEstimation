...
public class FrameReconstructor_v2 {
	...
	FrameReconstructor_v2(int l_sig, int n_sig, int k_max) {
	...
	public Jama.Matrix reconstruct(Jama.Matrix X){
		...
		if ( X.getRowDimension() == length_embedded && X.getColumnDimension() == width_embedded){
			...
			//Log.v("Frame Reconstruct", "Input: Signal length: "+ X.getRowDimension() + " | Signal width: " + X.getColumnDimension());
